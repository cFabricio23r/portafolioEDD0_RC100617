...
void procesarArchivoVuelos(string nombreArchivo){
    ...
    cout << "Se creo al siguiente grafo" << endl;
...
void procesarSolicitudVuelo(){
    ...
    if (codigosCiudades.count(ciudadDestino)== 0){
        cout << "Lo iento, no volamos hacia esa ciudad." << endl;
        return;
    }
    
    RutaBFS iteneratio(rutas, codigosCiudades[ciudadOrigen]);

    if (!iteneratio.existeCaminoHacia(codigosCiudades[ciudadDestino])){
        cout << "No se pudo encontrar un itinerario de vuelo." << endl;
    }
    else{
        cout << "Itinerario sugerido: " << endl;
        list<int> :: iterator it;
        list<int> conexiones = iteneratio.getRutaHacia(codigosCiudades[ciudadDestino]);
